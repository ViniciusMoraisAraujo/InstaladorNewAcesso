...
public class StandAloneExConfigHelperTests : IDisposable
{
    private readonly string _tempRoot;

    public StandAloneExConfigHelperTests()
    {
        _tempRoot = Path.Combine(Path.GetTempPath(), "StandAloneExTests_" + Guid.NewGuid().ToString("N"));
    }
    ...
    {
        var dir = Path.Combine(_tempRoot, "NewAcesso", "ControllerOffline", "WinService_Ex");
        Directory.CreateDirectory(dir);

        var result = StandAloneExConfigHelper.UpdateConfigAfterInstall(dir);
    ...
    {
        var shallowDir = Path.Combine(_tempRoot, "WinService_Ex");
        Directory.CreateDirectory(shallowDir);
        CreateMinimalConfig(Path.Combine(shallowDir, "PrimeAcesso.Controller.StandAloneEx.exe.config"));

        var result = StandAloneExConfigHelper.UpdateConfigAfterInstall(shallowDir);
    ...
    public void UpdateConfigAfterInstall_CorruptedXml_ReturnsFalse()
    {
        var dir = Path.Combine(_tempRoot, "NewAcesso", "ControllerOffline", "WinService_Ex");
        Directory.CreateDirectory(dir);
        File.WriteAllText(Path.Combine(dir, "PrimeAcesso.Controller.StandAloneEx.exe.config"), "invalid {{{");
    ...
    [Fact]
    public void UpdateConfig_WithAlternateNameEx_UpdatesSuccessfully()
    {
        var dir = Path.Combine(_tempRoot, "NewAcesso", "ControllerOffline", "WinService_Ex");
        Directory.CreateDirectory(dir);
        var configPath = Path.Combine(dir, "PrimeAcesso.StandAloneEx.exe.config");
        CreateMinimalConfig(configPath);

        var result = StandAloneExConfigHelper.UpdateConfig(dir, idConexao: "4");
        ...
        idNode!.GetAttribute("value").Should().Be("4");
    }
    ...
    public void UpdateConfig_WithTrailingSlash_ResolvesDbPathCorrectly()
    {
        var dir = Path.Combine(_tempRoot, "NewAcesso", "ControllerOffline", "WinService_Ex");
        Directory.CreateDirectory(dir);
        var configPath = Path.Combine(dir, "PrimeAcesso.Controller.StandAloneEx.exe.config");
        CreateMinimalConfig(configPath);

        var result = StandAloneExConfigHelper.UpdateConfig(dir + @"\");
