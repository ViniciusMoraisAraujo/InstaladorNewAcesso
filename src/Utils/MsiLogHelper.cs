...
public static class MsiLogHelper
{
    ...
    public static void DisplayLogAnalysis(string logFilePath)
    {
        ...
        var panel = new Panel(
                Align.Center(new Markup(analysis.ErrorSummary?.EscapeMarkup() ?? "[gray]Nenhuma análise disponível.[/]")))
            .Header("[yellow]\U0001f4cb DIAGNÓSTICO DA INSTALAÇÃO[/]")
        ...
        if (analysis.ErrorContext != null && analysis.ErrorContext.Length > 0)
        {
            ...
            {
                AnsiConsole.MarkupLine($"[gray]{line.EscapeMarkup()}[/]");
            }
        }
    }
}
