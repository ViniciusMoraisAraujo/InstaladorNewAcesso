...
public class GoogleDriveDownloader : IDisposable
{
    ...
    /// <summary>
    /// Test-only constructor that allows injecting a mock HttpClient.
    /// Does NOT override the timeout so the caller retains full control.
    /// </summary>
    public GoogleDriveDownloader(string apiKey, HttpClient httpClient)
    {
        _apiKey = apiKey;
        _http = httpClient;
    }
    ...
    private async Task DownloadFolderRecursiveAsync(string folderId, string localPath, IProgress<string>? progress)
    {
        ...
        do
        {
            ...
            var response = System.Text.Json.JsonSerializer.Deserialize<DriveListResponse>(json, JsonOptions);
